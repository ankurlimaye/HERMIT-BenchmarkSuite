          ...
          for (k = 0 ; k < kmax ; k++) {
            ...
            switch (isd[i]->info.fmt) {
              case 80: ivmin = -0x80;
                ...
                break;
              case 310:
              case 311: ivmin = -0x200;
                ivmax = 0x1ff;
                break;
              case 212: ivmin = -0x800;
                ivmax = 0x7ff;
                break;
              case 16:
              case 61:
              case 160: ivmin = -0x8000;
                ivmax = 0x7fff;
                break;
              case 24: ivmin = -0x800000;
                ivmax = 0x7fffff;
                break;
              default: ivmin = WFDB_SAMPLE_MIN;
...
static int r310(struct igdata *g) {
  ...
  switch (g->count++) {
    case 0: v = (g->data = r16(g)) >> 1;
      break;
    case 1: v = (g->datb = r16(g)) >> 1;
      break;
    case 2:
    default: g->count = 0;
...
static void w310(WFDB_Sample v, struct ogdata *g) {
  ...
  switch (g->count++) {
    case 0: g->data = (v << 1) & 0x7fe;
      break;
    case 1: g->datb = (v << 1) & 0x7fe;
      break;
    case 2:
    default: g->count = 0;
      g->data |= (v << 11);
  ...
  for (s = 0 ; s < nisig ; s++) {
    ...
    for (c = 0 ; c < is->info.spf ; c++, vector++) {
      switch (is->info.fmt) {
        ...
        default: *vector = v = is->samp += r8(ig);
          break;
