/*
--------------------------------------------------------------------------------
File: signal.c - Original: George Moody - 13 Apr 1989
                  Revised:              -  8 Mar 2019 (wfdblib 10.6.2)
...
#define w8(V, G) (((*(G->bp++) = (char) V)), \
                  (_l = (G->bp != G->be) ? 0 : ((_n = (G->bsize > 0) ? G->bsize : obsize), wfdb_fwrite((G->bp = G->buf), 1, _n, G->fp))))
...
static WFDB_Time fstrtim(char *string, WFDB_Frequency f) {
    ...
    switch (*string) {
        case 'c':
            return (cfreq > 0. ? (WFDB_Time)((strtod(string + 1, NULL) - bcount) * f / cfreq) : (WFDB_Time)(strtol(string + 1, NULL, 10)));
        case 'e':
            return ((in_msrec ? msnsamples : nsamples) *
                    (((gvmode & WFDB_HIGHRES) == WFDB_HIGHRES) ? ispfmax : 1));
        case 'f':
            return (WFDB_Time)(strtol(string + 1, NULL, 10) * f / ffreq);
        case 'i':
        ...
        case 'o':
            return (ostime);
        case 's':
            return ((WFDB_Time) strtol(string + 1, NULL, 10));
