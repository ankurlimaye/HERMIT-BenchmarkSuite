/*
--------------------------------------------------------------------------------
File: ecgmap.h - Original: George Moody - 8 Jun 1983
                  Revised: George Moody - 4 May 1999    (wfdblib 10.0.0)
ECG annotation code mapping macros
--------------------------------------------------------------------------------
wfdb: a library for reading and writing annotated waveforms (time series data)

These macros evaluate their arguments only once, so that they behave like functions with respect to
side-effects (e.g., `isqrs(x++)' is safe). With the exception of isann(), each macro uses a table;
to avoid wasting space in programs compiled from more than one source, try to keep all references to
these macros in a single source file so that multiple instances of the tables are not required. To
save even more space, simply define the unneeded macros before including this file (e.g., `#define
map1').
--------------------------------------------------------------------------------
...
#ifndef ammap
#define ammap(A) (('D' < (wfdb_mt = (A)) && wfdb_mt <= ']') ? wfdb_ammp[wfdb_mt - 'E'] : NOTQRS)
...
#ifndef mamap
#define mamap(A, S) (isann(A) ? (((wfdb_mt = wfdb_mamp[wfdb_mt]) == 'U' && (S) != -1) ? 'O' : wfdb_mt) : 'O')
