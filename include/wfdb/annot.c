/*
--------------------------------------------------------------------------------
File: annot.c - Original: George Moody - 13 Apr 1989
                 Revised: George Moody -  8 Mar 2019 (wfdblib 10.6.2)
WFDB library functions for annotations
--------------------------------------------------------------------------------
wfdb: a library for reading and writing annotated waveforms (time series data)

This file contains definitions of the following functions, which are not visible outside of this
file:
...
(Numbers in brackets in the list above indicate the first version of the WFDB library that included
the corresponding function. Functions not so marked have been included in all published versions of
the WFDB library.)

These functions, also defined here, are intended only for the use of WFDB library functions defined
elsewhere:
 wfdb_anclose  (closes all annotation files)
 wfdb_oaflush  (flushes output annotations)

Beginning with version 5.3, the functions in this file read and write annotation translation table
modifications as `modification labels' (`NOTE' annotations attached to sample 0 of signal 0). This
feature provides transparent support for custom annotation definitions in WFDB applications.
Previous versions of these functions, if used to read files containing modification labels, treat
them as ordinary NOTE annotations.

Simultaneous annotations attached to different signals (as indicated by the `chan' field) are
supported by version 6.1 and later versions. Annotations must be written in time order; simultaneous
annotations must be written in `chan' order. Simultaneous annotations are readable but not writable
by earlier versions.

Beginning in version 10.4.12, the canonical annotation order is given by the time, num, and chan
fields in that order. Thus simultaneous annotations may be attached to the same signal provided that
their num fields are unique.
--------------------------------------------------------------------------------
...
You may contact the author by e-mail (wfdb@physionet.org) or postal mail (MIT Room E25-505A,
Cambridge, MA 02139 USA). For updates to this software, please visit PhysioNet
...
static struct iadata {
    ...
    double tt;                       /* annotation time (MIT format only). This equals ann.time
                   unless a SKIP follows ann; in such cases, it is the time of the SKIP (i.e., the
                   time of the annotation following ann) */
    double ann_tt;                   /* unscaled annotation time of 'ann' */
    double pann_tt;                  /* unscaled annotation time of 'pann' */
    double prev_tt;                  /* unscaled time of the last annotation returned by getann */
    WFDB_Time prev_time;             /* sample number of the last annotation returned by getann */
} * *iad;
...
static struct oadata {
    ...
    char out_of_order;    /* if >0, one or more annotations written by putann are not in the
                   canonical (time, num, chan) order */
    char table_written;   /* if >0, table has been written */
} * *oad;
static WFDB_Frequency oafreq; /* time resolution in ticks/sec for newly created output annotators */

/* Local functions (for the use of other functions in this module only). */

#include <limits.h>

/* Round a double to the nearest WFDB_Time, with halfway cases always rounded up. (For example,
round_to_time(10.5) is 11, but round_to_time(-10.5) is -10.) */
...
static char modified[ACMAX + 1]; /* modified[i] is non-zero if setannstr() or setanndesc() has
                   modified the mnemonic or description for annotation type i */
    ...
    for (i = 0; i < nann; i++) { /* open the annotation files */
        ...
        switch (aiarray[i].stat) {
            ...
            case WFDB_AHA_READ: /* AHA-format input file */
                ...
                /* Try to figure out what format the file is in.  AHA-format files begin with a null
            byte and an ASCII character which is one of the legal AHA annotation codes other than
            '[' or ']'. MIT annotation files cannot begin in this way. */
    ...
    *annot = ia->ann;
    ...
    switch (ia->info.stat) {
            ...
            while (((ia->word = (unsigned) wfdb_g16(ia->file)) & CODE) >= PAMIN &&
                ...
                switch (ia->word & CODE) { /* process pseudo-annotations */
                    ...
                    case AUX:                  /* auxiliary information */
                        ...
                        /* Now read the data.  Note that an extra byte may be present in the
                annotation file to preserve word alignment; if so, this extra byte is read and then
                overwritten by the null in the second statement below. */
        ...
        case WFDB_AHA_READ:                  /* AHA-format input file */
            ...
            /* There is very limited space in AHA format files for auxiliary information, so no
                length byte is recorded; instead, we assume that if the first byte of auxiliary data
                is not null, that up to AUXLEN bytes may be significant. */
...
FINT putann(WFDB_Annotator n, WFDB_Annotation *annot) {
    ...
    switch (oa->info.stat) {
        ...
        default:
            /* Do not allow annotations to be written at the minimum or maximum possible time value.
        This prevents applications from inadvertently clamping annotations to the WFDB_Time range,
        which is almost always a mistake (for example, using a 32-bit 'mrgann' on a record longer
        than 2^31 samples.) In addition, encoding an annotation at time WFDB_TIME_MAX on a 64-bit
        system would require 2^32 SKIPs (24 GB), so it's better to catch such bugs beforehand. */
            if (t == WFDB_TIME_MIN || t == WFDB_TIME_MAX) {
                wfdb_error("putann: time overflow in annotation file %d\n", n);
                return (-1);
            }
            if (t > oa->ann.time) {
                /* A SKIP can represent a forward offset of at most 2^31-1, so if delta is larger
            than that, it needs to be represented by multiple SKIPs. */
                while (delta > MAXSKIP) {
                ...
                /* Likewise, a SKIP can represent a backward offset of at most 2^31 (minus 1 to
            account for the special handling of null annotations below.) */
                while (-delta > -MINSKIP) {
                    wfdb_p16(SKIP, oa->file);
            ...
            if (annot->anntyp == 0) {
                /* The caller intends to write a null annotation here, but putann must not write a
            word of zeroes that would be interpreted as an EOF.  To avoid this, putann writes a SKIP
            to the location just before the desired one;  thus annwd (below) is never 0. */
                wfdb_p16(SKIP, oa->file);
                wfdb_p32(delta - 1, oa->file);
                delta = 1;
            } else if (delta > MAXRR) {
                /* skip forward by more than MAXRR, or skip backward by any distance */
                wfdb_p16(SKIP, oa->file);
...
/* iannsettime: seek so that for the next annotation read from each input annotator, anntime >= t */
FINT iannsettime(WFDB_Time t) {
    int stat = 0, niavalid = niaf;
    WFDB_Annotation tempann;
    WFDB_Annotator i;

    /* Handle negative arguments as equivalent positive arguments. As an exception, WFDB_TIME_MIN
        indicates that we should rewind to the very beginning of the annotation file, even if there
        are annotations at negative time values. */
...
/* Functions for converting between anntyp values (annotation codes defined in <ecgcode.h>),
    mnemonics (short strings, usually only one character), and descriptive strings.

    There are two sets of mnemonics (cstring[] and astring[]) and one set of descriptive strings
    (tstring[]) defined below. The two sets of mnemonics are identical by default.

    The conversion functions (ecgstr and annstr) translate the annotation code specified by their
    argument into a mnemonic. Illegal or undefined codes are translated into decimal numerals
    surrounded by brackets (e.g., `[55]'). The mnemonics returned by annstr (those defined in
    astring[]) may be modified either by setannstr or by the presence of modification labels in an
    input annotation file.Those returned by ecgstr (defined in cstring[]) are usually the same, but
    they can be modified only by setecgstr, and not by the presence of modification labels as for
    annstr. The intent is that ecgstr should be used rather than annstr only when it is necessary
    that a fixed set of mnemonics be used, independent of any modification labels. The functions
    strecg and annstr perform the inverse of these translations.

    The functions anndesc and setanndesc are similar to annstr and setannstr, except that they use
    the descriptive strings (tstring[]). */
...
FINT setecgstr(int code, char *string) {
    if (NOTQRS <= code && code <= ACMAX) {
        ...
        cstring[code] = NULL; /* This statement (and the corresponding statements in setannstr and
                    setanndesc) leak memory if the function is called more than once with the same
                    value for code -- which is unlikely. */
            ...
            if (dosort) {
                if (system(NULL) != 0) {
                    wfdb_error(
                            ...
                            oa->info.name);
                    /* The option '-r.' tells sortann to change its WFDB path to the current
            directory (i.e, it should only attempt to read the annotation file that we just created,
            and should not look elsewhere, regardless of the WFDB environment variable.)

            Older versions of sortann interpret '-r.' as equivalent to '-r', and will search for the
            given annotation file within the WFDB path. */
                    wfdb_asprintf(&cmdbuf, "sortann -r. %s -a %s", oa->rname, oa->info.name);
...
/* Semi-private functions
   ...
   These functions wrap the macros defined in <ecgmap.h>. They are intended to simplify maintenance
   of WFDB wrappers generated using SWIG, which cannot wrap macros automatically.
*/
