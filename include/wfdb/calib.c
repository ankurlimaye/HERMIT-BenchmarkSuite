/*
--------------------------------------------------------------------------------
File: calib.c - Original: George Moody -  4 Jul 1991
                 Revised: George Moody - 18 Nov 2013 (wfdblib 10.5.21)
...
(version 6.0 used \n only). These are not compatible with calopen() version 6.0, although
calibration files written by newcal() version 6.0 can be read successfully by all versions of
calopen(). This change was made so that calibration files can be more easily viewed and edited
under MS-DOS.

Beginning with version 8.3, calopen() uses the default calibration file name (DEFWFDBCAL, defined in
wfdblib.h) if passed a NULL argument and if the WFDBCAL environment variable is not set.
--------------------------------------------------------------------------------
...
/* Calibration list entries each contain a WFDB_Calinfo record (crec) and a forward pointer (next).
*/
static struct cle {
...
/* Function calopen reads the specified calibration file;  if called with a NULL argument, it takes
   the value of the environment variable WFDBCAL to be the name of the calibration file.  If the
   calibration file name does not begin with "+", calopen empties the calibration list first;
   otherwise, the "+" is discarded before attempting to open the file, which may be in any directory
   in the WFDB path. If the file can be read, its contents are appended to the calibration list. */
FINT calopen(char *cfname) {
    ...
    if (*cfname == '+') /* don't empty the calibration list */
        ...
        flushcal(); /* empty the calibration list */
    ...
    while (wfdb_fgets(buf, 127, cfile)) {
        ...
        /* This line appears to be a correctly formatted entry. Allocate memory for a calibration
           list entry. */
...
/* Function getcal attempts to find a calibration record which matches the supplied desc and units
   strings.  If the sigtype field of the record is a prefix of or an exact match for the desc
   string, and if the units field of the record is an exact match for the units string, the record
   matches.  If either desc or units is NULL, it is ignored for the purpose of finding a match. If a
   match is found, it is copied into the caller's WFDB_Calinfo structure. The caller must not write
   over the storage addressed by the desc and units fields. */
...
/* Function putcal appends the caller's WFDB_Calinfo structure to the end of the calibration list.
*/
...
/* Function newcal generates a calibration file based on the contents of the calibration list. */
FINT newcal(char *cfname) {
    ...
    for (this_cle = first_cle; this_cle; this_cle = this_cle->next) {
        ...
        switch (this_cle->caltype & (~WFDB_DC_COUPLED)) {
            case WFDB_CAL_SQUARE:
                pulsetype = "square";
                break;
            case WFDB_CAL_SINE:
                pulsetype = "sine";
                break;
            case WFDB_CAL_SAWTOOTH:
                pulsetype = "sawtooth";
                break;
            default:
                pulsetype = "undefined";
