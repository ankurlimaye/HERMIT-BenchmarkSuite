/*
--------------------------------------------------------------------------------
File: wabp.c - Original: Wei Zong     - 23 Oct 1998
...
signal. This version of wabp works best with ABP signals sampled at 125 Hz, but it can analyze ABPs
sampled at any frequency using on-the-fly resampling provided by the WFDB library. 'wabp' has been
optimized for adult human ABPs. For other ABPs, it may be necessary to experiment with the input
sampling frequency and the time constants indicated below.

`wabp' can process records containing any number of signals, but it uses only one signal for ABP
pulse detection (by default, the lowest-numbered signal labelled `ABP', `ART', or `BP'; this can be
changed using the `-s' option, see below).

For a usage summary, see the help text at the end of this file. The input record may be in any of
the formats readable by the WFDB library, and it may be anywhere in the WFDB path (in a local
...
You may contact the author by e-mail (wzong@mit.edu) or postal mail (MIT Room E25-505, Cambridge,
...
int sig = -1;   /* signal number of signal to be analyzed (initial value forces search for
                            ABP, ART, or BP signal) */
...
int main(int argc, char **argv) {
    ...
    int ExpectPeriod; /* if no ABP pulse is detected over this period, the threshold is
                            automatically reduced to a minimum value;  the threshold is restored
                            upon a detection */
