...
static int DGifGetWord(FILE *File, short int *Word);
...
int DGifGetRecordType(GifFileType *GifFile, GifRecordType *Type) {
  ...
  switch (Buf) {
    case ',': *Type = IMAGE_DESC_RECORD_TYPE;
      break;
    case '!': *Type = EXTENSION_RECORD_TYPE;
      break;
    case ';': *Type = TERMINATE_RECORD_TYPE;
      break;
...
static int DGifGetWord(FILE *File, short int *Word) {
  ...
  *Word = (unsigned) ((((unsigned) c[1]) << 8) + c[0]);
  return GIF_OK;
}
